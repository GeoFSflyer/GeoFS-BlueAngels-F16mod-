// ==UserScript==
// @name         GeoFS F-16 Addon
// @namespace    https://github.com/ArjanKw/GeoFS-BlueAngels/
// @version      1.7.0
// @description  Improves the cockpit with a new HUD and custom MFDs, adjustable seat height and more. Ported from the F-18/F-16 Addon for the F-15 Eagle.
...
(function () {
  ...
  const VERSION = '1.7.0';
  const F15_AIRCRAFT_ID = '3591';

  const FLIGHT_RECORDER_MIN_VERSION = '1.2.0';
  const RAD_TO_DEG = 180 / Math.PI;
  const CAMERA_TO_HUD_DISTANCE_M = 0.92;
  const F15_OPTIONS_STORAGE_KEY = 'F15Options';
  const F15_WPN_STATE_STORAGE_KEY = 'F15WpnState';
  ...
  class F15MainPlugin {
    ...
    constructor() {
      ...
      this.hudModule = new F15HudModule();
      ...
      this.lastMfdRecoveryTick = -999;
            ...
            // TODO (F-15 port): The MFD position below is copied from earlier ports and must be tuned to the F-15 cockpit.
      //   Tune position/rotation/scale visually in GeoFS to fit the F-16 cockpit.
      //   - position[0]=X (left/right), position[1]=Y (up/down), position[2]=Z (depth)
      //   - rotation[0] = panel tilt towards pilot
      //   - scale = MFD size on panel
this.addMfd({
name: 'MAIN',
// TODO (F-15 port): tune this position in GeoFS F-15 cockpit.
// Approx: lower-left corner of main panel.
position: [-0.20, 4.8, 0.9],
rotation: [8, 0, 0],
...
defaultPageTitle: 'NAV'
  ...
  function saveWpnStateToStorage() {
    ...
    window.localStorage.setItem(F15_WPN_STATE_STORAGE_KEY, JSON.stringify(payload));
  }

  function loadWpnStateFromStorage() {
    const raw = window.localStorage.getItem(F15_WPN_STATE_STORAGE_KEY);
    if (!raw) return;
  ...
  function readOptions() {
    ...
    const raw = window.localStorage.getItem(F15_OPTIONS_STORAGE_KEY);
    if (!raw) {
      optionStoreCache = {};
      return optionStoreCache;
    }
  ...
  function writeOptions(options) {
    ...
    window.localStorage.setItem(F15_OPTIONS_STORAGE_KEY, JSON.stringify(payload));
    return true;
  }
  ...
  // Checks if the currently selected aircraft is the F-16.
  function isF15Active() {
    return (window.geofs?.aircraft?.instance?.id ?? '') === F15_AIRCRAFT_ID;
  }
  ...
  function createDefaultChecklistModule() {
    ...
    module.addChecklist({
      type: 'PROC',
      title: 'Before Taxi',
      items: ['N/A (F-16)', 'Arr. Hook UP', 'N/A (F-16)', 'N/A (F-16)', 'Flaps MAN', 'Canopy AS DESIRED', 'Recording AS DESIRED', 'Taxi REQUESTED'],
      completed: false
    });
    module.addChecklist({
      type: 'PROC',
    ...
    module.addChecklist({
      type: 'OPS',
      title: 'IFF Codebook',
      items: ['Say \'IFF [CS] - Code [NO.]\'', 'Respond with \'IFF [Code]\'', 'Ã¢â€Å’Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Â', 'Ã¢â€â€š  01: 457 Ã¢â€â€š  02: 701 Ã¢â€â€š ', 'Ã¢â€â€š  03: 337 Ã¢â€â€š  04: 241 Ã¢â€â€š ', 'Ã¢â€â€š  05: 612 Ã¢â€â€š  06: 135 Ã¢â€â€š ', 'Ã¢â€â€š  07: 402 Ã¢â€â€š  08: 984 Ã¢â€â€š ', 'Ã¢â€â€š  09: 264 Ã¢â€â€š  10: 753 Ã¢â€â€š ', 'Ã¢â€â€š  11: 755 Ã¢â€â€š  12: 588 Ã¢â€â€š ', 'Ã¢â€â€š  13: 284 Ã¢â€â€š  14: 000 Ã¢â€â€š ',, 'Ã¢â€â€Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€Ëœ'],
      completed: false
    });
    module.addChecklist({
      type: 'OPS',
      title: 'Targeting Pod - A/G',
      items: ['Flightplan OPEN', 'Target MARK AS WAYPOINT', 'Entry INGRESS FROM SOUTH', 'Heading 0Ã‚Â°', 'Flightplan SELECT TARGET WP', 'MFD SWITCH TO TGP', 'MODE/FREQ AS DESIRED', 'FOV WIDE', 'View ADJUST', 'FOV NARROW'],
      completed: false
    });
    module.addChecklist({
      type: 'OPS',
      title: 'Targeting Pod - A/A',
      items: ['MFD SWITCH TO RDR', 'Radar ON', 'Foo AS DESIRED', 'MFD SWITCH TO NAV', 'A/C SELECT', 'MFD SWITCH TO TGP', 'Entry INGRESS FROM SOUTH', 'Heading 0Ã‚Â°', 'Lock SET TO TRK', 'MODE / FOV AS DESIRED'],
      completed: false
    });
    module.addChecklist({
      type: 'OPS',
  ...
  class CommunicationModule {
    ...
    trimLine(text, maxChars = 72) {
      ...
      return `${value.slice(0, Math.max(0, maxChars - 1))}Ã¢â‚¬Â¦`;
    }
  ...
  rightButtons: [
    {
      key: 'SLEW_UP', label: 'Ã¢â€ â€˜', states: [''], stateIndex: 0,
      onClick: ({ page }) => page && page._updateSlew(0, 1)
    },
    {
      key: 'SLEW_DOWN', label: 'Ã¢â€ â€œ', states: [''], stateIndex: 0,
      onClick: ({ page }) => page && page._updateSlew(0, -1)
    },
    {
      key: 'SLEW_LEFT', label: 'Ã¢â€ Â', states: [''], stateIndex: 0,
      onClick: ({ page }) => page && page._updateSlew(-1, 0)
    },
    {
      key: 'SLEW_RIGHT', label: 'Ã¢â€ â€™', states: [''], stateIndex: 0,
      onClick: ({ page }) => page && page._updateSlew(1, 0)
    },
    {
...
computeCameraOrientationRelativeToAircraft(camLLA, tgtLLA, aircraft) {
  ...
  function computeHudGeometry(w, h) {
    const hudVerticalFovDeg = 2 * Math.atan((F15HudModule.HUD_PHYSICAL_HEIGHT_M / 2) / CAMERA_TO_HUD_DISTANCE_M) * RAD_TO_DEG;
    const pixelsPerDeg = h / hudVerticalFovDeg;
    const hudPhysicalWidthM = F15HudModule.HUD_PHYSICAL_HEIGHT_M * (w / h);
    ...
    const cameraOffsetPx = cameraOffsetDeg * pixelsPerDeg * F15HudModule.HUD_PARALLAX_GAIN;
  ...
  // ---------------------------------------------------------------------------
  // Draw calls Ã¢â‚¬â€ geen fillText-prototype nodig; ctx is de gewone canvas context
  // ---------------------------------------------------------------------------
  ...
  function drawAoaText(ctx, w, h, aoa) {
    ...
    ctx.textAlign = 'left';
    ctx.textBaseline = 'middle';
    ctx.fillText(`ÃŽÂ± ${aoa.toFixed(1)}`, w * 0.716, h * 0.93);
  ...
  // Pitch ladder Ã¢â‚¬â€ gebruikt ctx.fillText rechtstreeks (geen prototype-omweg).
  ...
  function drawLeftReadouts(ctx, mach, gValue, aoa, maxGValue, autopilot, w, h) {
    ...
    ctx.fillText(`ÃŽÂ± ${aoa.toFixed(1)}`, x, y2);
  ...
  // ---------------------------------------------------------------------------
  // F-16 HUD renderer Ã¢â‚¬â€ volledige custom draw
  // ---------------------------------------------------------------------------

  function renderF16Hud(renderer) {
  ...
  class F15HudModule {
    ...
    install() {
      ...
      renderers.genericHUD = function (renderer) {
        if (!isF15Active() || getOption('HUD', 'HUD', 'F-15') === 'DEFAULT') {
          return self.originalRenderer.call(this, renderer);
        }
        renderF16Hud(renderer);
      };
    ...
    ensureLoaded() {
      if (getOption('HUD', 'HUD', 'F-15') === 'DEFAULT') {
        this.restore();
        return true;
      }
  ...
  class CameraModule {
    ...
    static CAMERA_UP_BUTTON_ID = 'f16-hud-camera-up';
    static CAMERA_DOWN_BUTTON_ID = 'f16-hud-camera-down';
    static CAMERA_VIEW_BUTTON_ID = 'f16-cockpit-view-cycle';
    ...
    constructor(helperModule) {
      ...
      this.cockpitViewControlsWrapperId = 'f16-cockpit-view-controls';
    ...
    installCameraControls() {
      if (document.getElementById('f16-hud-camera-controls')) return true;
      if (!this.helperModule) return false;

      const wrapper = document.createElement('div');
      wrapper.id = 'f16-hud-camera-controls';
    ...
    removeCameraControls() {
      this.helperModule?.removePadControl('f16-hud-camera-controls');
    }
  ...
  class ControlModule {
    constructor(helperModule = null) {
      ...
      this.probeControlsWrapperId = 'f16-probe-controls';
      this.registerDefaultControls();
    }

    registerDefaultControls() {
      // SYS.REFUELING control removed: F-16 has no refueling probe / doors.
        ...
        parts: [
          {
            ...
            channels: {
              canopyRotXDeg: { OPEN: 30, CLOSED: 0 },
              canopyRotYDeg: { OPEN: 0, CLOSED: 0 },
              canopyRotZDeg: { OPEN: 0, CLOSED: 0 }
            }
          }
        ]
      });
    ...
    installProbeControls() {
      // F-16: No refueling probe Ã¢â‚¬â€ probe controls are hidden.
      return false;

      if (!this.helperModule || !isF15Active()) return false;
      ...
      const openButton = this.createProbePadButton('OPEN', 'f16-probe-open', () => {
      ...
      const probeLabel = this.createProbePadButton('PROBE', 'f16-probe-label', () => {}, {
        marginTop: '-9px',
        borderRadius: '0',
        borderTop: '0',
        cursor: 'default',
      ...
      const closeButton = this.createProbePadButton('CLOSE', 'f16-probe-close', () => {
  ...
  class MfdModule {
    ...
    ensureUiState() {
      if (!addonRuntime.mfdUiStates[this.slotName]) {
        addonRuntime.mfdUiStates[this.slotName] = new F15MfdUiState();
      }
  ...
  function createAddonApi() {
    return {
      ...
      hud: {
        ...
        getMode: () => getOption('HUD', 'HUD', 'F-15'),
        setMode: (mode) => {
        ...
        isCustomEnabled: () => getOption('HUD', 'HUD', 'F-15') !== 'DEFAULT'
      },
      mfd: {
        ...
        addDisplay: (config = {}) => {
          if (!addonRuntime.mainPlugin) {
            addonRuntime.mainPlugin = new F15MainPlugin();
  ...
  if (window.F15Addon?.lifecycle?.stop) {
    window.F15Addon.lifecycle.stop();
  }

  window.F15Addon = createAddonApi();
  window.F15Addon.lifecycle.start();
})();

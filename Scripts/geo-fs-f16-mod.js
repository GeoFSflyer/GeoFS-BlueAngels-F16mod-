...
// @version      1.7.0
// @description  Improves the cockpit with a new HUD and custom MFDs, adjustable seat height and more. Ported from the F-18 Addon for the F-16 Fighting Falcon.
...
(function () {
  ...
  const VERSION = '1.7.0';
  const F16_AIRCRAFT_ID = '7';

  const FLIGHT_RECORDER_MIN_VERSION = '1.2.0';
  const RAD_TO_DEG = 180 / Math.PI;
  const CAMERA_TO_HUD_DISTANCE_M = 0.92;
  const F16_OPTIONS_STORAGE_KEY = 'F16Options';
  const F16_WPN_STATE_STORAGE_KEY = 'F16WpnState';
  ...
  class F16MainPlugin {
    ...
    constructor() {
      ...
      this.hudModule = new F16HudModule();
      ...
      this.lastMfdRecoveryTick = -999;
            ...
            // TODO (F-16 port): The MFD positions below are copied from the F-18.
...
this.addMfd({
  ...
  function saveWpnStateToStorage() {
    ...
    window.localStorage.setItem(F16_WPN_STATE_STORAGE_KEY, JSON.stringify(payload));
  }

  function loadWpnStateFromStorage() {
    const raw = window.localStorage.getItem(F16_WPN_STATE_STORAGE_KEY);
    if (!raw) return;
  ...
  function readOptions() {
    ...
    const raw = window.localStorage.getItem(F16_OPTIONS_STORAGE_KEY);
    if (!raw) {
      optionStoreCache = {};
      return optionStoreCache;
    }
  ...
  function writeOptions(options) {
    ...
    window.localStorage.setItem(F16_OPTIONS_STORAGE_KEY, JSON.stringify(payload));
    return true;
  }
  ...
  function isF16Active() {
    return (window.geofs?.aircraft?.instance?.id ?? '') === F16_AIRCRAFT_ID;
  }
  ...
  function createDefaultChecklistModule() {
    ...
    module.addChecklist({
      type: 'OPS',
      title: 'IFF Codebook',
      items: ['Say \'IFF [CS] - Code [NO.]\'', 'Respond with \'IFF [Code]\'', 'â”Œâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”', 'â”‚  01: 457 â”‚  02: 701 â”‚ ', 'â”‚  03: 337 â”‚  04: 241 â”‚ ', 'â”‚  05: 612 â”‚  06: 135 â”‚ ', 'â”‚  07: 402 â”‚  08: 984 â”‚ ', 'â”‚  09: 264 â”‚  10: 753 â”‚ ', 'â”‚  11: 755 â”‚  12: 588 â”‚ ', 'â”‚  13: 284 â”‚  14: 000 â”‚ ',, 'â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜'],
      completed: false
    });
    module.addChecklist({
      type: 'OPS',
      title: 'Targeting Pod - A/G',
      items: ['Flightplan OPEN', 'Target MARK AS WAYPOINT', 'Entry INGRESS FROM SOUTH', 'Heading 0Â°', 'Flightplan SELECT TARGET WP', 'MFD SWITCH TO TGP', 'MODE/FREQ AS DESIRED', 'FOV WIDE', 'View ADJUST', 'FOV NARROW'],
      completed: false
    });
    module.addChecklist({
      type: 'OPS',
      title: 'Targeting Pod - A/A',
      items: ['MFD SWITCH TO RDR', 'Radar ON', 'Foo AS DESIRED', 'MFD SWITCH TO NAV', 'A/C SELECT', 'MFD SWITCH TO TGP', 'Entry INGRESS FROM SOUTH', 'Heading 0Â°', 'Lock SET TO TRK', 'MODE / FOV AS DESIRED'],
      completed: false
    });
    module.addChecklist({
      type: 'OPS',
  ...
  rightButtons: [
    {
      key: 'SLEW_UP', label: 'â†‘', states: [''], stateIndex: 0,
      onClick: ({ page }) => page && page._updateSlew(0, 1)
    },
    {
      key: 'SLEW_DOWN', label: 'â†“', states: [''], stateIndex: 0,
      onClick: ({ page }) => page && page._updateSlew(0, -1)
    },
    {
      key: 'SLEW_LEFT', label: 'â†', states: [''], stateIndex: 0,
      onClick: ({ page }) => page && page._updateSlew(-1, 0)
    },
    {
      key: 'SLEW_RIGHT', label: 'â†’', states: [''], stateIndex: 0,
      onClick: ({ page }) => page && page._updateSlew(1, 0)
    },
    {
...
computeCameraOrientationRelativeToAircraft(camLLA, tgtLLA, aircraft) {
  ...
  function computeHudGeometry(w, h) {
    const hudVerticalFovDeg = 2 * Math.atan((F16HudModule.HUD_PHYSICAL_HEIGHT_M / 2) / CAMERA_TO_HUD_DISTANCE_M) * RAD_TO_DEG;
    const pixelsPerDeg = h / hudVerticalFovDeg;
    const hudPhysicalWidthM = F16HudModule.HUD_PHYSICAL_HEIGHT_M * (w / h);
    ...
    const cameraOffsetPx = cameraOffsetDeg * pixelsPerDeg * F16HudModule.HUD_PARALLAX_GAIN;
  ...
  // ---------------------------------------------------------------------------
  // Draw calls â€” geen fillText-prototype nodig; ctx is de gewone canvas context
  // ---------------------------------------------------------------------------
  ...
  // Pitch ladder â€” gebruikt ctx.fillText rechtstreeks (geen prototype-omweg).
  ...
  // ---------------------------------------------------------------------------
  // F-16 HUD renderer â€” volledige custom draw
  // ---------------------------------------------------------------------------
  ...
  class F16HudModule {
    ...
    install() {
      ...
      renderers.genericHUD = function (renderer) {
        if (!isF16Active() || getOption('HUD', 'HUD', 'F-16') === 'DEFAULT') {
    ...
    ensureLoaded() {
      if (getOption('HUD', 'HUD', 'F-16') === 'DEFAULT') {
        this.restore();
        return true;
      }
  ...
  class ControlModule {
    ...
    installProbeControls() {
      // F-16: No refueling probe â€” probe controls are hidden.
      return false;

      if (!this.helperModule || !isF16Active()) return false;
  ...
  class MfdModule {
    ...
    ensureUiState() {
      if (!addonRuntime.mfdUiStates[this.slotName]) {
        addonRuntime.mfdUiStates[this.slotName] = new F16MfdUiState();
      }
  ...
  function createAddonApi() {
    return {
      ...
      hud: {
        ...
        getMode: () => getOption('HUD', 'HUD', 'F-16'),
        setMode: (mode) => {
        ...
        isCustomEnabled: () => getOption('HUD', 'HUD', 'F-16') !== 'DEFAULT'
      },
      mfd: {
        ...
        addDisplay: (config = {}) => {
          if (!addonRuntime.mainPlugin) {
            addonRuntime.mainPlugin = new F16MainPlugin();
  ...
  if (window.F16Addon?.lifecycle?.stop) {
    window.F16Addon.lifecycle.stop();
  }

  window.F16Addon = createAddonApi();
  window.F16Addon.lifecycle.start();
})();

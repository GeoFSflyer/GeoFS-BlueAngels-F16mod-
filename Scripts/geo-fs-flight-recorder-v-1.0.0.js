...
(function () {
  ...
  function applyPrecisionVisualFilter(track, lla, htr, dt) {
    const strength = clamp(Number(ultraStrength) || 0, 0, 100) / 100;
    const dtMs = clamp(Number(dt) || 16, 1, 80);
    const dtSec = dtMs / 1000;

    const softDeadband = (v, band) => {
      const a = Math.abs(v);
      if (a <= band) return 0;
      return Math.sign(v) * (a - band);
    };
    const clampSym = (v, lim) => clamp(v, -lim, lim);

    if (!track._precision) {
      track._precision = {
        posLla: [lla[0], lla[1], lla[2]],
        velLla: [0, 0, 0],
        posHtr: [htr[0], htr[1], htr[2]],
        velHtr: [0, 0, 0],
        prevMeasLla: [lla[0], lla[1], lla[2]],
        prevMeasHtr: [htr[0], htr[1], htr[2]]
      };
      return { lla, htr };
    }

    const st = track._precision;
    const meters = metersPerDeg(st.posLla[0]);
    const mLon = Math.max(1, meters.mLon);

    const alpha = lerp(0.42, 0.18, strength);
    const beta = lerp(0.075, 0.02, strength);
    const velFollow = lerp(0.22, 0.08, strength);
    const velDamp = Math.exp(-dtMs / lerp(140, 300, strength));

    const deadbandMeters = lerp(0.015, 0.10, strength);
    const deadbandDeg = lerp(0.025, 0.10, strength);

    const maxCorrMeters = lerp(0.55, 0.22, strength);
    const maxCorrDeg = lerp(1.8, 0.7, strength);

    const vMeasLat = (lla[0] - st.prevMeasLla[0]) / dtSec;
    const vMeasLon = angleDeltaDeg(st.prevMeasLla[1], lla[1]) / dtSec;
    const vMeasAlt = (lla[2] - st.prevMeasLla[2]) / dtSec;
    const vMeasHdg = angleDeltaDeg(st.prevMeasHtr[0], htr[0]) / dtSec;
    const vMeasPit = angleDeltaDeg(st.prevMeasHtr[1], htr[1]) / dtSec;
    const vMeasRol = angleDeltaDeg(st.prevMeasHtr[2], htr[2]) / dtSec;

    st.velLla[0] = lerp(st.velLla[0], vMeasLat, velFollow);
    st.velLla[1] = lerp(st.velLla[1], vMeasLon, velFollow);
    st.velLla[2] = lerp(st.velLla[2], vMeasAlt, velFollow);
    st.velHtr[0] = lerp(st.velHtr[0], vMeasHdg, velFollow);
    st.velHtr[1] = lerp(st.velHtr[1], vMeasPit, velFollow);
    st.velHtr[2] = lerp(st.velHtr[2], vMeasRol, velFollow);

    const predLat = st.posLla[0] + st.velLla[0] * dtSec;
    const predLon = st.posLla[1] + st.velLla[1] * dtSec;
    const predAlt = st.posLla[2] + st.velLla[2] * dtSec;
    const predHdg = st.posHtr[0] + st.velHtr[0] * dtSec;
    const predPit = st.posHtr[1] + st.velHtr[1] * dtSec;
    const predRol = st.posHtr[2] + st.velHtr[2] * dtSec;

    let rLat = lla[0] - predLat;
    let rLon = angleDeltaDeg(predLon, lla[1]);
    let rAlt = lla[2] - predAlt;
    let rHdg = angleDeltaDeg(predHdg, htr[0]);
    let rPit = angleDeltaDeg(predPit, htr[1]);
    let rRol = angleDeltaDeg(predRol, htr[2]);

    rLat = softDeadband(rLat, deadbandMeters / meters.mLat);
    rLon = softDeadband(rLon, deadbandMeters / mLon);
    rAlt = softDeadband(rAlt, deadbandMeters);
    rHdg = softDeadband(rHdg, deadbandDeg);
    rPit = softDeadband(rPit, deadbandDeg);
    rRol = softDeadband(rRol, deadbandDeg);

    rLat = clampSym(rLat, maxCorrMeters / meters.mLat);
    rLon = clampSym(rLon, maxCorrMeters / mLon);
    rAlt = clampSym(rAlt, maxCorrMeters);
    rHdg = clampSym(rHdg, maxCorrDeg);
    rPit = clampSym(rPit, maxCorrDeg);
    rRol = clampSym(rRol, maxCorrDeg);

    st.posLla[0] = predLat + alpha * rLat;
    st.posLla[1] = predLon + alpha * rLon;
    st.posLla[2] = predAlt + alpha * rAlt;
    st.posHtr[0] = predHdg + alpha * rHdg;
    st.posHtr[1] = predPit + alpha * rPit;
    st.posHtr[2] = predRol + alpha * rRol;

    st.velLla[0] = (st.velLla[0] + (beta * rLat) / dtSec) * velDamp;
    st.velLla[1] = (st.velLla[1] + (beta * rLon) / dtSec) * velDamp;
    st.velLla[2] = (st.velLla[2] + (beta * rAlt) / dtSec) * velDamp;
    st.velHtr[0] = (st.velHtr[0] + (beta * rHdg) / dtSec) * velDamp;
    st.velHtr[1] = (st.velHtr[1] + (beta * rPit) / dtSec) * velDamp;
    st.velHtr[2] = (st.velHtr[2] + (beta * rRol) / dtSec) * velDamp;

    st.velLla[0] = clamp(st.velLla[0], -0.0015, 0.0015);
    st.velLla[1] = clamp(st.velLla[1], -0.0015, 0.0015);
    st.velLla[2] = clamp(st.velLla[2], -180, 180);
    st.velHtr[0] = clamp(st.velHtr[0], -540, 540);
    st.velHtr[1] = clamp(st.velHtr[1], -540, 540);
    st.velHtr[2] = clamp(st.velHtr[2], -540, 540);

    st.prevMeasLla[0] = lla[0];
    st.prevMeasLla[1] = lla[1];
    st.prevMeasLla[2] = lla[2];
    st.prevMeasHtr[0] = htr[0];
    st.prevMeasHtr[1] = htr[1];
    st.prevMeasHtr[2] = htr[2];

    return {
      lla: [st.posLla[0], st.posLla[1], st.posLla[2]],
      htr: [st.posHtr[0], st.posHtr[1], st.posHtr[2]]
    };
  }
  ...
  function poseAt(track, i1, i2, f, dt) {
    ...
    if (planeSplineOn && track.xy?.length && track.base) {
      ...
      if (motionSmoothOn) {
        ...
        const alpha = 1 - Math.exp(-dt / Math.max(1, Math.min(motionTauMs, 60)));
    ...
    return applyPrecisionVisualFilter(track, [lat, lon, alt], [hdg, pit, rol], dt);
  }
